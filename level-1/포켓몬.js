...
function solution(nums) {
  const have = [];
  nums.map((num) => {
    if (!have.includes(num)) have.push(num);
  });
  return have.length >= nums.length / 2 ? nums.length / 2 : have.length;
}

//using new Set
function solution2(nums) {
  const max = nums.length / 2;
  const onlyOne = [...new Set(nums)];
  return onlyOne.length >= max ? max : onlyOne.length;
}
